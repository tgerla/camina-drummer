...
#        self._draw_beats(drum_machine, d)
    ...
    def _draw_pattern_name(self, drum_machine, d):
        d.text((10, 160), drum_machine.get_current_pattern_name(), font=self.regularFont, fill=(255, 255, 255))

    def _draw_tempo_display(self, drum_machine, d):
        d.text((170, 18), "%d" % drum_machine.tempo, font=self.regularFont, fill=(255, 255, 255))
        ...
        d.text((SCREEN_WIDTH/2-8, 30), "%s" % drum_machine.pattern_signature, font=self.regularFont, fill=tsColor)

    def _draw_pattern_display(self, drum_machine, d):
        d.text((180, 170), "%d" % drum_machine.current_pattern_idx, font=self.boldFont, fill=(255, 255, 255))
...
class PyGameDisplay(Display):
    ...
    def flip(self, drawingContext):
        img = pygame.image.frombuffer(self.buffer.tobytes(), self.buffer.size, self.buffer.mode)
